...
def hydrophobicfun(atom1, atom2, dist=5):
    ...
    if (
        (atom1.get_name() not in no_inter_atom and "H" not in atom1.get_name()) and
...
def cationpifun(atom1, atom2, resid1, resid2, dist=6):
    ...
    if (
        (((resid1.get_resname() in ["LYS", "ARG"]
           and resid2.get_resname() in ["PHE", "TYR", "TRP"]) and
          len(atom1.get_name()) > 1 and atom1.get_name() != "CA"
          and len(atom2.get_name()) > 1 and atom2.get_name() != "CA"
          and "H" not in atom1.get_name() and "H" not in atom2.get_name()) or
         ((resid2.get_resname() in ["LYS", "ARG"]
           and resid1.get_resname() in ["PHE", "TYR", "TRP"])
          and len(atom1.get_name()) > 1 and atom1.get_name() != "CA"
          and len(atom2.get_name()) > 1 and atom2.get_name() != "CA"
          and "H" not in atom1.get_name() and "H" not in atom2.get_name()))
