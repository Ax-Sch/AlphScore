library(tidyverse)
library(readxl)
list_uniprot_ids<-read_tsv("resources_validation/mapping_genename_uniprot.tsv")
#sheets<-excel_sheets("Source.xlsx")
sheets<-c("ADRB2","BRCA1","CALM1", "HRAS", "MAPK1", "P53",  "PTEN", "SUMO1", "TPK1", "TPMT", "UBE2I")
scores_conc<-tibble()

for (gene in sheets){
  scores <- read_excel("resources_validation/Source.xlsx", sheet=gene) %>% 
...
#MSH2
scores <- read_excel("resources_validation/MSH2_Jia_2020/1-s2.0-S0002929720304390-mmc2.xlsx", sheet="TableS5_AllVariants")%>%
...
#Abeta
scores <- read_excel("resources_validation/Abeta_Seuma_2020/elife-63364-supp4-v2.xlsx", sheet="1 aa change")%>%
...
RASH_path="resources_validation/RASH_Bandaru_2017_regulated/elife-27810-supp1-v2.xlsx"
...
#VKOR1
scores <- read_excel("resources_validation/VKOR1_Chiasson_2020_activity/elife-58026-fig1-data1-v1.xlsx")%>%
...
files<-paste0("resources_validation/feature_files/", list_uniprot_ids$uniprot_id, "_w_AFfeatures.csv.gz") 
...
table(values_joined$Uniprot_acc_split)
table(scores_conc$uniprot_id)

values_joined[values_joined$Uniprot_acc_split=="P05067",] %>% select(variant, aaref, DMS)
spearmans_joined<-tibble()
for (un_ID in unique(values_joined$Uniprot_acc_split)){
  temp_values_joined<-values_joined %>% filter(Uniprot_acc_split==un_ID)
...
cor.test(as.double((scores_conc %>% filter(gene_dms=="ADRB2", DMS=="DMS_0"))$DMS_val), as.double((scores_conc %>% filter(gene_dms=="ADRB2", DMS=="DMS_0.15"))$DMS_val), method="spearman")
cor.test((values_joined %>% filter(gene_dms=="ADRB2", DMS=="DMS_0"))$DMS_val, (values_joined %>% filter(gene_dms=="ADRB2", DMS=="DMS_0.15"))$DMS_val, method="spearman")

dir.create("data/validation_set")
setwd("data/validation_set")

fwrite(x=values_joined, file="values_joined.csv.gz")
