...
cv18_to_21_CV<- variants%>%filter(cv18_to_21_CV_test==1)
gene_no<-unique(variants$gene)
pure_cv18_to_21_gene<- variants%>%filter(pure_cv18_to_21_gene==1)
train1<-variants%>%filter(!(pure_cv18_to_21_gene==TRUE) & gnomadSet==TRUE)
test1<-variants%>%filter(clinvar_no_cv21to18_no_gnomad==TRUE)
test2<-variants %>% filter(cv18_to_21_CV_test==TRUE)
rest_train1<-variants%>%filter((pure_cv18_to_21_gene==TRUE) & gnomadSet==TRUE)
